...
function movePlayer(player) {
    ...
    let inBounds = true;
    const radiusOffset = c.hardWalls ?  (player.radius / 2) + borderCalc: 0;
    if ((player.x + radiusOffset) > c.gameWidth - borderCalc) {
        player.x = c.gameWidth - borderCalc - radiusOffset;
        inBounds = false;
    }
    if ((player.y + radiusOffset) > c.gameHeight - borderCalc) {
        player.y = c.gameHeight - borderCalc - radiusOffset;
        inBounds = false;
    }
    if ((player.x - radiusOffset) < borderCalc) {
        player.x = borderCalc + radiusOffset;
        inBounds = false;
    }
    if ((player.y - radiusOffset) < borderCalc) {
        player.y = borderCalc + radiusOffset;
        inBounds = false;
    }

    if (c.deadlyBorder && !inBounds) {
